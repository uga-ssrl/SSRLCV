...
namespace ssrlcv{
  ...
  */
  struct MatchSet{
    std::shared_ptr<Unity<KeyPoint>> keyPoints;
    std::shared_ptr<Unity<MultiMatch>> matches;
  };
  ...
  */
  ...
  class MatchFactory{
  private:
    std::shared_ptr<Unity<Feature<T>>> seedFeatures;///<\brief Features generated from false seed image
  public:
     ...
     * \see Unity
     */
    void setSeedFeatures(std::shared_ptr<Unity<Feature<T>>> seedFeatures);
     ...
     * \see Unity
     */
    void validateMatches(std::shared_ptr<ssrlcv::Unity<Match>> matches);
    /**
     * \brief Discards matches flagged as invalid
     ...
     * \see Unity
     */
    void validateMatches(std::shared_ptr<ssrlcv::Unity<DMatch>> matches);
    /**
     * \brief Discards matches flagged as invalid
     ...
     * \see Unity
     */
    void validateMatches(std::shared_ptr<ssrlcv::Unity<uint2_pair>> matches);
     ...
     * \see Unity
     */
    void refineMatches(std::shared_ptr<ssrlcv::Unity<DMatch>> matches, float threshold);
    /**
     * \brief Discards matches with distance over an absolute threshold.
     ...
     * \see Unity 
     */
    void sortMatches(std::shared_ptr<ssrlcv::Unity<DMatch>> matches);
    /**
     ...
     * \returns std::shared_ptr<ssrlcv::Unity<Match>> an array of simplified matches
     * \see Unity
     */
    std::shared_ptr<ssrlcv::Unity<Match>> getRawMatches(std::shared_ptr<ssrlcv::Unity<DMatch>> matches);
    /**
     * \brief Returns Unity of simplified Match structs.
     * \details This simply converts every element in the Unity 
     ...
     * \see Unity
     */
    std::shared_ptr<ssrlcv::Unity<Match>> getRawMatches(Unity<FeatureMatch<T>>* matches);
     ...
     * \returns std::shared_ptr<ssrlcv::Unity<float>> an array same length as features with distances associated 
     */
    std::shared_ptr<ssrlcv::Unity<float>> getSeedDistances(std::shared_ptr<Unity<Feature<T>>> features);

    /**
     * \brief Generates Matches between Feature<Descriptor> when Descriptor::distProtocol() is implemented
     */
    std::shared_ptr<ssrlcv::Unity<Match>> generateMatches(std::shared_ptr<Image> query, std::shared_ptr<Unity<Feature<T>>> queryFeatures, std::shared_ptr<Image> target, std::shared_ptr<Unity<Feature<T>>> targetFeatures, std::shared_ptr<ssrlcv::Unity<float>> seedDistances = nullptr);
    /**
     ...
     */
    std::shared_ptr<ssrlcv::Unity<Match>> generateMatchesConstrained(std::shared_ptr<Image> query, std::shared_ptr<Unity<Feature<T>>> queryFeatures, std::shared_ptr<Image> target, std::shared_ptr<Unity<Feature<T>>> targetFeatures, float epsilon, float fundamental[3][3], std::shared_ptr<ssrlcv::Unity<float>> seedDistances = nullptr);

    /**
     * \brief Generates DMatches between Feature<Descriptor> when Descriptor::distProtocol() is implemented
     */
    std::shared_ptr<ssrlcv::Unity<DMatch>> generateDistanceMatches(std::shared_ptr<Image> query, std::shared_ptr<Unity<Feature<T>>> queryFeatures, std::shared_ptr<Image> target, std::shared_ptr<Unity<Feature<T>>> targetFeatures, std::shared_ptr<ssrlcv::Unity<float>> seedDistances = nullptr);
    /**
     * \brief Generates DMatches between Feature<Descriptor> when Descriptor::distProtocol() is implemented with epipolar constaints
     * \warning If bad fundamental matrix between two images, matches will be very bad. 
     */
    std::shared_ptr<ssrlcv::Unity<DMatch>> generateDistanceMatchesConstrained(std::shared_ptr<Image> query, std::shared_ptr<Unity<Feature<T>>> queryFeatures, std::shared_ptr<Image> target, std::shared_ptr<Unity<Feature<T>>> targetFeatures, float epsilon, float fundamental[3][3], std::shared_ptr<ssrlcv::Unity<float>> seedDistances = nullptr);
    
    /**
     * \brief Generates FeatureMatch<Descriptor>s between Feature<Descriptor> when Descriptor::distProtocol() is implemented
     */
    Unity<FeatureMatch<T>>* generateFeatureMatches(std::shared_ptr<Image> query, std::shared_ptr<Unity<Feature<T>>> queryFeatures, std::shared_ptr<Image> target, std::shared_ptr<Unity<Feature<T>>> targetFeatures, std::shared_ptr<ssrlcv::Unity<float>> seedDistances = nullptr);
    /**
     ...
     */
    Unity<FeatureMatch<T>>* generateFeatureMatchesConstrained(std::shared_ptr<Image> query, std::shared_ptr<Unity<Feature<T>>> queryFeatures, std::shared_ptr<Image> target, std::shared_ptr<Unity<Feature<T>>> targetFeatures, float epsilon, float fundamental[3][3], std::shared_ptr<ssrlcv::Unity<float>> seedDistances = nullptr);


    std::shared_ptr<ssrlcv::Unity<uint2_pair>> generateMatchesIndexOnly(std::shared_ptr<Image> query, std::shared_ptr<Unity<Feature<T>>> queryFeatures, std::shared_ptr<Image> target, std::shared_ptr<Unity<Feature<T>>> targetFeatures, std::shared_ptr<ssrlcv::Unity<float>> seedDistances = nullptr);
    std::shared_ptr<ssrlcv::Unity<uint2_pair>> generateMatchesConstrainedIndexOnly(std::shared_ptr<Image> query, std::shared_ptr<Unity<Feature<T>>> queryFeatures, std::shared_ptr<Image> target, std::shared_ptr<Unity<Feature<T>>> targetFeatures, float epsilon, float fundamental[3][3], std::shared_ptr<ssrlcv::Unity<float>> seedDistances = nullptr);
    ...
    */
    MatchSet generateMatchesExaustive(std::vector<std::shared_ptr<Image>> images, std::vector<std::shared_ptr<Unity<Feature<T>>>> features, bool ordered = true, float estimatedOverlap = 0.0f);
  
  };

  std::shared_ptr<ssrlcv::Unity<Match>> generateDiparityMatches(uint2 querySize, std::shared_ptr<ssrlcv::Unity<unsigned char>> queryPixels, uint2 targetSize, std::shared_ptr<ssrlcv::Unity<unsigned char>> targetPixels, 
  ...
  * \{
  */
  void writeMatchFile(std::shared_ptr<ssrlcv::Unity<Match>> matches, std::string pathToFile, bool binary = false);
  void writeMatchFile(MatchSet multiview_matches, std::string pathToFile, bool binary = false);
  std::shared_ptr<ssrlcv::Unity<Match>> readMatchFile(std::string pathToFile);
  /** \} */

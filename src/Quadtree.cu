...
*/
template<typename T>
__device__ __host__ Quadtree<T>::Node::Node(){
  this->key = -1;
  this->dataIndex = -1;
  this->numElements = 0;
  this->center = {-1,-1};
...
template<typename T>
__device__ __host__ Quadtree<T>::Vertex::Vertex(){
  this->loc = {-1,-1};
  for(int i = 0; i < 4; ++i) this->nodes[i] = -1;
  this->depth = -1;
}
template<typename T>
__device__ __host__ Quadtree<T>::Edge::Edge(){
  this->vertices = {-1,-1};
...
template<typename T>
Quadtree<T>::Quadtree(uint2 imageSize, ssrlcv::Unity<T>* data){
  this->nodes = nullptr;
  this->edges = nullptr;
  this->vertices = nullptr;
  this->data = data;
...
template<typename T>
void Quadtree<T>::generateLeafNodes(int depth){
...
template<typename T>
void Quadtree<T>::generateParentNodes(){

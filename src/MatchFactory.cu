...
// void MatchFactory::refineMatches(MatchSet* matchSet, float cutoffRatio){
...
//   }
// }
// void MatchFactory::refineMatches(SubPixelMatchSet* matchSet, float cutoffRatio){
//   if(this->cutoffRatio == 0.0f){
//     std::cout<<"ERROR not cutoff ratio set for refinement"<<std::endl;
//     exit(-1);
//   }
...
__device__ __forceinline__ float ssrlcv::calcElucid_SIFTDescriptor(const unsigned char a[128], const unsigned char b[128]){
  float dist = 0.0f;
  for(int i = 0; i < 128; ++i){
...
__device__ __forceinline__ int ssrlcv::findSubPixelContributer(const int2 &loc, const int &width){
